...
可以根据项目需求选择上述任一或多个唤醒词进行模型训练。每个唤醒词都经过优化，适合在ESP32-S3芯片上运行。
...
| - | 6月26日开发 | -| 通过负例数据的设计，解决了静音高误触发问题，在mac上实时推理既快又准，可演示； | 2025-06-26 完成 https://github.com/qbox/xrobot/pull/8 |
| ⬜ | ESP芯片适配 | 2025-06-26 | 尚未完成 | 将ONNX模型转为ESP芯片格式，在实体机上测试 |
| ✅ | 效果提升 | 2025-06-28 | 正例（唤醒词）加入了噪音，负例新增了静音和极小噪音，测试集准确率提升到98% | 为TTS合成的训练数据加入噪音，模拟真实环境 |

## 推理技术架构

```
音频输入 → 环形缓冲区 →    MFCC特征提取 →   模型推理 →     置信度平滑 →  能量门控 → 检测结果
    ↓          ↓             ↓             ↓            ↓          ↓          ↓
  PyAudio   AudioBuffer   MFCCExtractor  ONNXRuntime   历史平滑    rms能量    阈值判断
```
